...
analyze_clade_evolution <- function(clade_name, clade_data) {
  ...
  china_data <- clade_data %>%
    filter((!is.na(cc) & cc == "CN") | (!is.na(country) & country == "China"))
  global_ex_china_data <- clade_data %>%
    filter(!((!is.na(cc) & cc == "CN") | (!is.na(country) & country == "China")))
  global_incl_data <- clade_data
  
  if (nrow(china_data) == 0) {
    warning(paste("No regional China records found for clade:", clade_name,
                  "Bypassing analysis."))
    return(NULL)
  }
  
  dataset_list <- list(
    "Global_incl" = list(matrix = make_abundance_matrix(global_incl_data, "early_interval"),
                         data = global_incl_data),
    "Global_excl" = list(matrix = make_abundance_matrix(global_ex_china_data, "early_interval"),
                         data = global_ex_china_data),
    "China"       = list(matrix = make_abundance_matrix(china_data, "early_interval"),
                         data = china_data)
  )
  ...
  for (set_name in names(dataset_list)) {
    ...
    if (ncol(mat) == 0 || nrow(mat) == 0) {
      stop(paste("Empty abundance matrix generated for", set_name, "in", clade_name))
    }
    ...
    stage_results[[set_name]] <- list(iNEXT_out = iNEXT_out, div_095 = div_095,
                                      raw = raw, mat = mat)
  }
  ...
  build_stage_long <- function(global_set, label_global) {
    build_ts <- function(div_metric) {
      data.frame(stage = stage_order) %>%
        ...
        left_join(stage_results[[global_set]]$raw %>% rename(global_raw = raw_richness),
                  by = "stage") %>%
        left_join(stage_results$China$raw %>% rename(china_raw = raw_richness),
                  by = "stage") %>%
  ...
  main_plots <- list(
    ...
    stage_min = build_plot(stage_plot_excl$plot_data_min, seg_stage_min_excl, region_colours,
                           paste0("Stage-level genus richness (q = 0, C = ",
                                  round(min_cov_stage, 3), ")"), "mid_age", TRUE),
    ...
    bin_min   = build_plot(filter(bin_plot_excl, coverage_type == "mincov"), seg_bin_min_excl,
                           region_colours,
                           paste0("10-Myr bin genus richness (q = 0, C = ",
                                  round(min_cov_bin, 3), ")"), "age")
  )
  ...
  for (name in names(main_plots)) {
    ggsave(paste0("Main_Figures/", clade_name, "_Main_", name, ".jpg"),
           plot = main_plots[[name]], width = 8, height = 5, dpi = 600)
  }
  for (name in names(supp_plots)) {
    ggsave(paste0("Supplementary_Figures/", clade_name, "_Supp_", name, ".jpg"),
           plot = supp_plots[[name]], width = 8, height = 5, dpi = 600)
  }
...
# ---- Run analyses ----
if (nrow(bivalvia_df) > 0) {
  analyze_clade_evolution(clade_name = "Bivalvia", clade_data = bivalvia_df)
}
if (nrow(echino_df) > 0) {
  analyze_clade_evolution(clade_name = "Echinodermata", clade_data = echino_df)
}
